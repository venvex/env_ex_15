package dp.singleton.implementation.singleThreaded;

import dp.singleton.implementation.base.Singleton;


/***
    TODO 1.0: non thread safe Singleton - implementation
        - implement the Singleton design pattern focusing on the theoretical model of the design pattern

*/


public class NonThreadSafeSingleton  extends Singleton {


        /***
        TODO 1.1:
            - Declare and initialize all the instance or class variables that are strictly needed for the Singleton pattern implementation
        */
        private static NonThreadSafeSingleton nonThreadSafeInstance = null;

         /***
        TODO 1.2: class constructor
            - Set the appropriate access modifier for the constructor.
            - Directly calling the constructor should be possible only from within the class definition.
        */
         private NonThreadSafeSingleton()
            {
                super("nonThreadSafe");
            }


        /***
        TODO 1.3: method area
            - Declare and define all the instance or class methods that are required for the Singleton pattern implementation
        */
        public static NonThreadSafeSingleton getInstance ()
        {
            if(nonThreadSafeInstance==null)
            {
                nonThreadSafeInstance = new NonThreadSafeSingleton();
            }
            return nonThreadSafeInstance;

        }


}
